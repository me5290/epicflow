...
// ================================ @FXML 필드 ================================ //
    @FXML private ProgressBar playerHpBar;                                  // 플레이어 HP 바
    @FXML private ProgressBar playerMpBar;                                  // 플레이어 MP 바
    @FXML private ProgressBar playerexp;                                    // 플레이어 경험치 바
    @FXML private ProgressBar slimeHp,goblinHp,minoHp,dragonHp;             // 몬스터 HP 바
    @FXML private AnchorPane attacklist;                                    // 공격 UI 화면
    @FXML private HBox btnlist;
    @FXML private Pane exitalert;                                           // 도망 화면
    @FXML private Button invenBtn;                                          // 인벤토리 버튼
    @FXML private AnchorPane inventory;                                     // 인벤토리 화면
    ...
    @FXML private Button exitstatbtn;                                       // 도망 버튼
    @FXML private AnchorPane statPaneview;                                  // 스탯 창 화면
    ...
    @FXML private Label playerName,playerHp,playerMaxHp,playerMp,playerMaxMp,playerExp,playerMaxExp,playerHeatBox,playerLevel;      // 플레이어 관련 라벨
    @FXML private Label slimeName,slimeHeatBox,slimeNowHp,slimeMaxHp,goblinName,goblinHeatBox,goblinNowHp,goblinMaxHp,minoName,minoHeatBox,minoNowHp,minoMaxHp,dragonName,dragonHeatBox,dragonNowHp,dragonMaxHp;    // 몬스터 관련 라벨
    @FXML private Pane slimeArea,goblinArea,minoArea,dragonArea;    // 몬스터별 PANE
    @FXML private Button villageBtn;
    @FXML private Button statdownbtn;               // 스탯 창 관련 버튼
    @FXML private Button statupbtn;                 // 스탯 창 관련 버튼
    @FXML private TextField statupdown;             // 스탯 창 관련 텍스트필드
    @FXML private Text stPoint;                     // 스텟 포인트 표시 텍스트
    @FXML private Button powertext;                 // 스탯 창 관련 버튼
    @FXML private Pane win;                         // 플레이어 승리 시 화면
    @FXML private Pane lose;                        // 플레이어 패배 시 화면
    @FXML private Text strtest,dextest,wistest;     // 스탯 창 현재 플레이어 능력치 표시

// ================================ 필드 ================================ //
    ArrayList<PlayerDto> playerInfor = PlayerDao.getInstance().playerInfor();       // 플레이어 정보 배열 변수
    ArrayList<MonsterDto> monsterInfor = MonsterDao.getInstance().monsterInfor();   // 몬스터 정보 배열 변수
    int memberNum = LoginController.memberNum;          // 로그인 한 회원 번호 가져오기
    int monsterNum = VillageController.monsterNum;      // 사냥터에 따른 몬스터 번호 가져오기
    PlayerDto player = new PlayerDto();         // 로그인 한 플레이어 정보 저장 변수
    MonsterDto monster = new MonsterDto();      // 몬스터 정보 저장 변수
    Random random = new Random();               // 랜덤 함수 호출
    boolean turnState = true;                   // 턴 상태
    double playerPercent;                       // 플레이어 퍼센트
    double monsterPercent;                      // 몬스터 퍼센트
    int playerDecrease;                         // 플레이어 최대 체력 변수
    int playerRenewal;                          // 플레이어 현재 체력 변수
    int monsterDecrease;                        // 몬스터 최대 체력 변수
    int monsterRenewal;                         // 몬스터 현재 체력 변수
...
// ============================== 메소드 ============================== //
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        try {
            ...
            playerHp1.setText(Integer.toString(player.getHp()));                // 스탯 창 표시

            // 플레이어 최대체력 출력
            playerMaxHp.setText(Integer.toString(player.getMhp()));
            playerMaxHp1.setText(Integer.toString(player.getMhp()));            // 스탯 창 표시

            // 플레이어 마나 출력
            playerMp.setText(Integer.toString(player.getMp()));
            playerMp1.setText(Integer.toString(player.getMp()));                // 스탯 창 표시

            // 플레이어 최대마나 출력
            playerMaxMp.setText(Integer.toString(player.getMmp()));
            playerMaxMp1.setText(Integer.toString(player.getMmp()));            // 스탯 창 표시
            ...
            System.out.println("[ ※ 안내 : initialize 오류입니다. ※ ]" + e);
        }
    }
    ...
    // ==== 몬스터 체력 0일때 종료 및 DB에 데이터 저장 메소드
    ...
    public void villageBtn(){
        ...
        if (result){
            try {
                ...
                System.out.println("[ ※ 안내 : 마을 화면전환 오류입니다. ※ ]" + e);
            }
        }
    }
    ...
    // ==== 플레이어 스탯 창 작동 시 DB 업데이트
    public void playerSave(){
        player.setMhp(player.getMhp());                     // 플레이어 최대 체력
        player.setHp(player.getHp());                       // 플레이어 현재 체력
        player.setMmp(player.getMmp());                     // 플레이어 최대 마나
        player.setMp(player.getMp());                       // 플레이어 현재 마나
        player.setLevel(player.getLevel());                 // 플레이어 현재 레벨
        player.setExp(player.getExp());                     // 플레이어 현재 경험치
        player.setMoney(player.getMoney());                 // 플레이어 현재 돈
        player.setStatpoint(player.getStatpoint());         // 플레이어 현재 스탯포인트
        player.setPower(player.getPower());                 // 플레이어 현재 공격력
        player.setDefence(player.getDefence());             // 플레이어 현재 방어력
        player.setSkillpower(player.getSkillpower());       // 플레이어 현재 스킬데미지
        player.setStr(player.getStr());                     // 플레이어 현재 능력치 힘
        player.setDex(player.getDex());                     // 플레이어 현재 능력치 민첩
        player.setWis(player.getWis());                     // 플레이어 현재 능력치 지능
        player.setEva(player.getEva());                     // 플레이어 현재 능력치 회피
        player.setSpd(player.getSpd());                     // 플레이어 현재 능력치 속도
        player.setMno(player.getMno());                     // 플레이어 현재 회원번호
    }

    // ==== 플레이어 스챗 창 클릭 및 스텟 포인트 분배 시 플레이어 상태 표시
    public void playerStatusUpdeate(){
        stPName.setText(player.getPname());                                 // 플레이어 이름 표시
        ...
        stPLovel.setText(Integer.toString(player.getLevel()));              // 플레이어 레벨 표시
